...
> = ObservableQueryFields<Data, Variables> & {
  data: Data | undefined
  error?: ApolloError
  loading: boolean
...
export interface Actions {
  ...
  modules: {
    setData: (
      data: { key: string; data: Partial<QueryModuleState<any>> }
    ) => void
  }
}
...
export const actions: ActionsType<State, Actions> = {
  ...
  updateQuery: (params: QueryProps) => state => {
    state.modules[params.key].observable!.setOptions(
      extractWatchQueryOptions(params)
    )
  },
  startQuerySubscription: (key: string) => (state, actions) => {
    ...
    const previousData =
      current.loading || current.error ? moduleState.previousData : current.data
    actions.modules.setData({
      key,
      data: {
        previousData,
...
function getQueryResult<Data, Variables>(
  state: QueryModuleState<Data>
  ...
  const data = loading
    ...
    : error
      ? (state.observable!.getLastResult() || {}).data
      : currentResult.data
...
function getObservableQueryFields<Data, Variables>(
  observable: ObservableQuery<Data>
) {
  const fields = {
...
export function Query<Data, Variables>(
  props: QueryProps<Data, Variables>,
  children: any
) {
  return (
    state: { apollo: apollo.State },
    actions: { apollo: apollo.Actions }
  ) => {
    const moduleState: QueryModuleState<Data> | void =
      state.apollo.query.modules[props.key]
    if (!moduleState) {
      actions.apollo.initQuery(props)
      return
    }
    const vnode = resolveNode(
      props.render(getQueryResult(moduleState), children),
      state,
      actions
    )
